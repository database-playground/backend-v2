...
func (_q *GroupQuery) QueryScopeSets() *ScopeSetQuery {
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withScopeSets: _q.withScopeSets.Clone(),
...
func (_q *GroupQuery) WithScopeSets(opts ...func(*ScopeSetQuery)) *GroupQuery {
	query := (&ScopeSetClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withScopeSets = query
	return _q
}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withScopeSets; query != nil {
		if err := _q.loadScopeSets(ctx, query, nodes,
			func(n *Group) { n.Edges.ScopeSets = []*ScopeSet{} },
			func(n *Group, e *ScopeSet) { n.Edges.ScopeSets = append(n.Edges.ScopeSets, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedScopeSets {
		if err := _q.loadScopeSets(ctx, query, nodes,
...
func (_q *GroupQuery) loadScopeSets(ctx context.Context, query *ScopeSetQuery, nodes []*Group, init func(*Group), assign func(*Group, *ScopeSet)) error {
...
func (_q *GroupQuery) WithNamedScopeSets(name string, opts ...func(*ScopeSetQuery)) *GroupQuery {
	query := (&ScopeSetClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedScopeSets == nil {
		_q.withNamedScopeSets = make(map[string]*ScopeSetQuery)
	}
	_q.withNamedScopeSets[name] = query
	return _q
}
